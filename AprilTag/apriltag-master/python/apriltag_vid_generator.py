# apriltag_vid_generator.py
# 04/20/2020

# get filename of most recent video in logs directory
# open videocapture object of file
# create video writer object to make new video
# get frame of video
# convert frame to grayscale
# run apriltag detection on grayscale
# combine apriltag_gray with original frame
# write frame into videowriter object
...
def main():
    ...
    orig_filename = max(glob.iglob('logs/*.avi'), key=os.path.getctime)

    print("Most recent flight video: " + orig_filename)

    new_filename = orig_filename[:-4] + '_april'

    fourcc = cv2.VideoWriter_fourcc(*'MJPG') # *'MJPG' for .avi
    vid_out = cv2.VideoWriter((new_filename + '.avi'),fourcc, 30, (960,720))
    
    cap = cv2.VideoCapture(orig_filename)

    logfile = open((new_filename + '.csv'), "w")
    ...
    while(cap.isOpened()):
        ...
        if (ret == True):
            sys.stdout.write("\rGenerating timestep %i" %count)
            sys.stdout.flush()
            gray_frame = cv2.cvtColor(orig_frame, cv2.COLOR_BGR2GRAY)
            detections, dimg = det.detect(gray_frame, return_image=True)
            new_frame = orig_frame // 2 + dimg[:, :, None] // 2
            pose = -42
            for _, detection in enumerate(detections):
                pose, e0, e1 = det.detection_pose(detection,options.camera_params,options.tag_size)
                _draw_pose(new_frame,options.camera_params,options.tag_size,pose)
                #sys.stdout.write("\rPose %4.2f %4.2f %4.2f" %(pose[0,3], pose[1,3], pose[2,3]))
            #sys.stdout.flush()
            vid_out.write(new_frame)
            detected = not isinstance(pose, int)
            if (detected):
